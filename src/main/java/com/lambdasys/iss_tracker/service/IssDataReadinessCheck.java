...
public class IssDataReadinessCheck implements HealthIndicator {

    private final IssPositionCacheService cacheService;
    ...
    private String formatPosition() {
        var position = cacheService.getLatestPosition();
        if (position != null) {
            return String.format("LAT: %.4f, LON: %.4f (atualizado em %s)",
                    position.getLatitude(),
                    position.getLongitude(),
                    position.getUpdateAt());
        }
